module ActiveRecord::Import::SQLServerAdapter
  ...
  MIN_VERSION_FOR_IMPORT = 2008.freeze
  # MIN_VERSION_FOR_UPSERT = "3.24.0".freeze
  SQLSERVER_LIMIT_COMPOUND_SELECT = 1000
  ...
  def supports_import?(current_version_year = version_year)
    if current_version_year >= MIN_VERSION_FOR_IMPORT
      true
    else
      false
    end
  ...
  def supports_on_duplicate_key_update?(current_version = sqlserver_version)
    false
  ...
  def insert_many( sql, values, _options = {}, *args ) # :nodoc:
    ...
    value_sets = ::ActiveRecord::Import::ValueSetsRecordsParser.parse(values,
      max_records: SQLSERVER_LIMIT_COMPOUND_SELECT)
